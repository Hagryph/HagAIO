...
-- Modules/Misc.lua
-- Miscellaneous helpers, each split into its own submodule of a thin "Misc" module.
-- The decompose-by-collaborator shape the codebase uses elsewhere (DB's RowStore/IndexManager split,
-- Class's per-spec submodules):
--   * Flight Timers (FlightTimers) — time each flight path and show a countdown both while in flight
--     and when hovering a flight point on the map. The RECORDER is ALWAYS on -- installed in the
--     submodule's OnInitialize (the framework's always-on init), it keeps building the database even
--     while the module/submodule is off. The on-screen DISPLAY is gated on the submodule being loaded.
--   * Sell Junk (SellJunk) — sell grey items at a vendor, automatically or via a button.
-- The parent module owns only the shared flight DB tables (a submodule can't contribute tables) and
-- the two submodule registrations; every feature's behaviour + settings live in its submodule.
...
-- ========================================================================================
-- The thin parent module: owns the shared flight DB tables (a submodule can't contribute tables)
-- and nothing else. Its settings page renders the two submodules' sections (by their titles). It is
-- OFF by default -- so by default nothing displays and no junk sells, but the always-on flight
-- recorder (FlightTimers:OnInitialize) still builds the database.
-- ========================================================================================
local Misc = Class.new("Misc", ns.Module)

local miscModule = Misc:New("Misc", {
    title = "Miscellaneous",
...
ns.ModuleManager:Register(miscModule)
