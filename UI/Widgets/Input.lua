...
function InputW:Initialize(parent, width)
    ...
    box:SetScript("OnEditFocusGained", function()
        box:SetBackdropColor(Theme.Unpack("controlHover"))
        box:SetBackdropBorderColor(Theme.Unpack("accent")); self:_UpdateHint()
    end)
    box:SetScript("OnEditFocusLost", function()
        box:SetBackdropColor(Theme.Unpack("control"))
        box:SetBackdropBorderColor(Theme.Unpack("borderStrong")); commit(); self:_UpdateHint()
    end)
...
function InputW:SetHint(text)
    ...
    if not p.hint then
        ...
        fs:SetPoint("LEFT", box, "LEFT", 10, 0)     -- match the input text inset (+ a hair)
