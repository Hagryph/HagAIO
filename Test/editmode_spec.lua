...
describe("EditMode:Apply", function()
    it("anchors at CENTER/UIParent from a saved CENTER row", function()
        local em, db = newEM({ ux = 0, uy = 0 })
        em:_SetPosition("c", { point = "CENTER", x = 12, y = -34 })
        local f = makeFrame()
        em:Apply({ frame = f, key = "c", default = { point = "CENTER", x = 0, y = 0 } })
        -- ClearAllPoints reset, then one SetPoint(point, UIParent, point, x, y)
        assert.are.equal(1, #f.points)
        assert.are.equal("CENTER", f.points[1][1])
        assert.are.equal(_G.UIParent, f.points[1][2])
        assert.are.equal("CENTER", f.points[1][3])
        assert.are.equal(12, f.points[1][4])
        assert.are.equal(-34, f.points[1][5])
    end)

    it("anchors at TOPLEFT/UIParent from a saved TOPLEFT row", function()
        local em, db = newEM({ ux = 0, uy = 0 })
        em:_SetPosition("t", { point = "TOPLEFT", x = -20, y = -450 })
        local f = makeFrame()
        em:Apply({ frame = f, key = "t", default = { point = "CENTER", x = 0, y = 0 } })
        assert.are.equal(1, #f.points)
        assert.are.equal("TOPLEFT", f.points[1][1])
        assert.are.equal(_G.UIParent, f.points[1][2])
        assert.are.equal("TOPLEFT", f.points[1][3])
        assert.are.equal(-20, f.points[1][4])
        assert.are.equal(-450, f.points[1][5])
    end)

    it("falls back to the registration default when no row is saved", function()
        local em = newEM({ ux = 0, uy = 0 })   -- nothing stored for "d"
        local f = makeFrame()
        em:Apply({ frame = f, key = "d", default = { point = "TOPLEFT", x = 5, y = 7 } })
        assert.are.equal(1, #f.points)
        assert.are.equal("TOPLEFT", f.points[1][1])   -- the default point, not CENTER
        assert.are.equal(5, f.points[1][4])
        assert.are.equal(7, f.points[1][5])
    end)
end)

describe("EditMode:IsEditing", function()
    it("flips true on entering edit mode and false on leaving", function()
        local em = newEM()
        assert.is_false(em:IsEditing())
        em:_OnEnter()
        assert.is_true(em:IsEditing())
        em:_OnExit()
        assert.is_false(em:IsEditing())
    end)
end)
